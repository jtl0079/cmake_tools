...
#       return variables
# ====================================
# RETURN_VAR_PREFIX = CMAKE_TOOLS_CORE_DEPENDENCY_STD_PIPELINE_DOWNLOAD_AND_UNZIP_FFMPEG_PREBUILT_AND_CREATE_FFMPEG_TARGET
#
...
function(cmake_tools_core_dependency_std_pipeline_download_and_unzip_ffmpeg_prebuilt_and_create_ffmpeg_target)	
    get_filename_component(_current_dir ${CMAKE_CURRENT_FUNCTION_LIST_FILE} PATH)
    include(${_current_dir}/../../backend/ffmpeg/fetch/cmake_tools_core_dependency_std_backend_download_ffmpeg_prebuilt.cmake)
    include(${_current_dir}/../../backend/ffmpeg/create/cmake_tools_core_dependency_std_backend_create_ffmpeg_target.cmake)
	# ====================================
	#		parameters
	# ====================================
    ...
    if(NOT CMAKE_TOOLS_CORE_DEPENDENCY_STD_BACKEND_DOWNLOAD_FFMPEG_PREBUILT_SUCCESS)
    ...
    get_filename_component(zip_filename ${CMAKE_TOOLS_CORE_DEPENDENCY_STD_BACKEND_DOWNLOAD_FFMPEG_PREBUILT_ZIP_PATH} NAME)
    string(REGEX REPLACE "\\.zip$" "" extracted_dir_name "${zip_filename}")
    set(extracted_dir "${CMAKE_TOOLS_CORE_DEPENDENCY_STD_BACKEND_DOWNLOAD_FFMPEG_PREBUILT_DOWNLOAD_DIR}/${extracted_dir_name}")
    
    # Skip if already extracted
    if(NOT EXISTS "${extracted_dir}")
        file(ARCHIVE_EXTRACT INPUT "${CMAKE_TOOLS_CORE_DEPENDENCY_STD_BACKEND_DOWNLOAD_FFMPEG_PREBUILT_ZIP_PATH}" 
             DESTINATION "${CMAKE_TOOLS_CORE_DEPENDENCY_STD_BACKEND_DOWNLOAD_FFMPEG_PREBUILT_DOWNLOAD_DIR}")
        if(NOT is_silent)
    ...
    cmake_tools_core_dependency_std_backend_create_ffmpeg_target(
	...
	#       return variables
	# ====================================
    set(RETURN_VAR_PREFIX "CMAKE_TOOLS_CORE_DEPENDENCY_STD_PIPELINE_DOWNLOAD_AND_UNZIP_FFMPEG_PREBUILT_AND_CREATE_FFMPEG_TARGET")
    
    # Set current scope variables (for printing)
    set(${RETURN_VAR_PREFIX}_ROOT "${extracted_dir}")
    set(${RETURN_VAR_PREFIX}_DOWNLOAD_DIR "${CMAKE_TOOLS_CORE_DEPENDENCY_STD_BACKEND_DOWNLOAD_FFMPEG_PREBUILT_DOWNLOAD_DIR}")
    set(${RETURN_VAR_PREFIX}_ZIP_PATH "${CMAKE_TOOLS_CORE_DEPENDENCY_STD_BACKEND_DOWNLOAD_FFMPEG_PREBUILT_ZIP_PATH}")
    ...
    set(${RETURN_VAR_PREFIX}_DOWNLOAD_DIR "${CMAKE_TOOLS_CORE_DEPENDENCY_STD_BACKEND_DOWNLOAD_FFMPEG_PREBUILT_DOWNLOAD_DIR}" PARENT_SCOPE)
    set(${RETURN_VAR_PREFIX}_ZIP_PATH "${CMAKE_TOOLS_CORE_DEPENDENCY_STD_BACKEND_DOWNLOAD_FFMPEG_PREBUILT_ZIP_PATH}" PARENT_SCOPE)
...
# Example 6: Fully customized configuration
# cmake_tools_core_dependency_std_pipeline_download_and_unzip_ffmpeg_prebuilt_and_create_ffmpeg_target(
#     FILENAME "ffmpeg-n8.1-latest-win64-gpl-shared-8.1.zip"
#     DOWNLOAD_DIR "${CMAKE_BINARY_DIR}/downloads"
#     TARGET_NAME "MyFFmpeg"
#     IS_GLOBAL_MODE TRUE
#     IS_SILENT_MODE FALSE
# )
#
